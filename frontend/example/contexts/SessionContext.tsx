...
export function SessionProvider({ children, defaultWorkingDirectory = '/home/xuyuan/workspace' }: SessionProviderProps) {
  ...
  // Use ref to keep currentSessionId value up to date
  ...
  const handleNotification = useCallback((notification: NotificationParams) => {
    ...
    switch (updateType) {
        ...
        if (chunk) {
          ...
          if (sessionId) {
            setSessions(prev => {
              ...
              // Only set if no title exists (use first message as title)
      ...
      case 'agent_thought_chunk':
        ...
        // 重置 loading 状态（使用函数式更新）
        setIsLoading(prev => prev ? false : prev);
        break;

      case 'tool_call':
        // Tool call - handle different tool name formats
        // Claude ACP format: tool data is directly on update, not in update.toolCall
  ...
  const handleHistory = useCallback((history: Array<{ type: NotificationUpdateType; update: any }>) => {
    ...
    history.forEach((item, index) => {
      ...
      if (!chunk && type !== 'tool_call') {
        return; // 在 forEach 中使用 return 代替 continue
      }
      ...
      switch (type) {
        ...
        case 'tool_call':
          // Extract tool name from history message
  ...
  useWebSocketMessage((message: WSMessage) => {
    ...
    switch (type) {
      ...
      case 'permission_request':
        // Stop loading when receiving permission request, allowing user to interact with permission dialog
        setIsLoading(false);
        break;
  ...
  // List sessions after connection is ready
  useEffect(() => {
